...
:black_small_square: *'location'*: Sends a location message. :earth_americas:
""", use_aliases=True)
            msg.body(response)
            responded = True
            ...
            if r.status_code == 200:
                ...
                for article in articles:
                    ...
                    if 'Z' in article['publishedAt']:
                        published_at = datetime.datetime.strptime(article['publishedAt'][:19]+'+00:00', "%Y-%m-%dT%H:%M:%S+00:00")
                    else:
                        published_at = datetime.datetime.strptime(article['publishedAt']+'+00:00', "%Y-%m-%dT%H:%M:%S%z+00:00")
                    result += """
*{}*
Read more: {}
